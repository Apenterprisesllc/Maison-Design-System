...
export const handlers = [
  ...
  http.post(`${BASE}/rest/v1/bookings`, async ({ request }) => {
    ...
    const row = {
      ...
      is_guest: Boolean(body.is_guest ?? false),
      guest_name: (body.guest_name as string | null) ?? null,
      guest_phone: (body.guest_phone as string | null) ?? null,
      guest_address: (body.guest_address as string | null) ?? null,
      created_at: now,
      updated_at: now,
    };
